...
python train.py --resume checkpoints/qa_demo.bin --steps 1500
```
...
`configs/debug.json` нужен для быстрой проверки. `qa_demo.json` — модель
96/64/2/4/192 на 1000 шагов и с `text_ratio=0.35`. `tiny.json` крупнее и автоматически в тестах не
запускается. Для увеличения модели меняйте `d_model`, `n_layers`, `n_heads`,
...
python chat.py --checkpoint checkpoints/qa_demo.bin --temperature 0.7 --top-k 20 --max-new-tokens 80
```
