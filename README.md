...
Обычный answer-only SFT использует `qa_prompt_weight: 0`. Для очень маленьких
моделей этого иногда недостаточно: они запоминают частые ответы, но плохо
различают формулировки запросов. Pipeline также разрешает явно задать небольшой
вес prompt, например `0.05` или `0.10`. Это гибрид causal LM и SFT, поэтому
значение нужно выбирать по независимой генерационной проверке, а не по train
loss. Ноль остаётся значением по умолчанию и правильной отправной точкой.
...
- создаёт `tokenizer_report.json` и проверяет Unicode-покрытие, tokens/byte,
  tokens/word и долю byte fallback;
...
На Windows Visual Studio, `cl.exe`, PyTorch и TensorFlow не требуются. Ядра из `cuda/kernels.cu` компилируются NVRTC при первом выборе CUDA в новом процессе и напрямую загружаются через NVIDIA Driver API. Если в CUDA Toolkit доступна cuBLAS, матричные умножения автоматически используют её; без cuBLAS остаётся рабочий встроенный kernel.
...
CUDA backend реализует forward и backward операции, matmul, attention, softmax, embedding, masked cross-entropy, gradient clipping и AdamW. Промежуточные тензоры, параметры модели, градиенты и моменты AdamW сохраняются в VRAM между связанными операциями; CPU-копия материализуется только тогда, когда она действительно нужна Python-коду или сохранению checkpoint. Завершённый autograd-граф освобождается после `backward`, пул временных блоков VRAM ограничен, а градиенты matmul используют transpose-флаги cuBLAS без промежуточных копий. RMSNorm и scaled causal softmax выполняются слитыми CUDA-ядрами forward/backward, поэтому attention не создаёт отдельный плотный causal mask. Граф autograd и цикл обучения остаются общими для всех backend.

Замер полного шага обучения с разбиением времени по фазам:
...
Conventional answer-only SFT uses `qa_prompt_weight: 0`. Very small models may
instead need a small explicit prompt loss such as `0.05` or `0.10` to learn the
request representation rather than only the marginal distribution of common
answers. The pipeline permits this hybrid causal-LM/SFT objective. Select the
weight with held-out generation tests, not training loss; zero remains the
...
assistant identities, measures tokenizer quality (including tokens per word
and raw-byte fallback), and verifies that every stage has the same model
architecture. Each stage writes `lineage.json` with
...
CUDA tensors, model parameters, gradients, and AdamW state remain resident in
VRAM across connected operations. A host copy is materialized only when Python
code or checkpoint serialization needs it. Completed autograd graphs are
released immediately, temporary VRAM caching is bounded, and common bias
operations use dedicated kernels. Matmul gradients use cuBLAS transpose flags
without materializing transposed matrices. RMSNorm and scaled causal softmax
use fused forward/backward CUDA kernels, so attention does not allocate a
separate dense causal mask. To benchmark a complete training step with
per-phase timings, run:
