...
На Windows x64 многопоточный C++ backend входит в пакет и устанавливается этой же командой автоматически. Отдельно ставить компилятор или собирать DLL не требуется.
...
Сторонние runtime-зависимости библиотеке не нужны. На Windows x64 после установки используется вложенный C++ backend; на остальных платформах без собранной нативной библиотеки доступен Python backend.
...
По умолчанию используется режим `auto`: если нативная библиотека найдена, тяжёлые операции выполняются в C++, иначе всё работает на Python. Установка через pip на Windows x64 уже содержит готовую DLL.

Ручная сборка на Windows нужна только при изменении C++-исходников:
